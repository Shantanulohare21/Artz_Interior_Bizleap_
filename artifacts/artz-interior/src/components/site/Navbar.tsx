  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
        scrolled
          ? "glass border-b border-border/60 py-2 shadow-sm"
    ...
    >
      <div className="mx-auto max-w-7xl px-6 lg:px-12 flex items-center justify-between gap-8">
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                key={l.to}
                href={l.to}
                className={`relative text-[13px] font-medium tracking-[0.15em] uppercase transition-colors ${
        ...
        <Link
          href="/contact"
          className={`hidden lg:inline-flex items-center gap-3 shrink-0 px-5 py-2.5 text-[11px] font-semibold tracking-[0.25em] uppercase transition-all duration-300 ${
            scrolled
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden glass border-t border-border/40"
          >
            <div className="px-6 py-8 flex flex-col gap-1">
              {/* Mobile logo */}
              <div className="mb-4 pb-4 border-b border-border/40">
                <img src="/assets/logo.png" alt="Art Interiorz" className="h-12 w-auto" />
              </div>
              {links.map((l, i) => {
                const isActive = location === l.to;
                return (
                  ...
                  >
                    <Link
                      href={l.to}
                      className={`block py-3 font-display text-2xl tracking-wide border-b border-border/40 transition-colors ${
